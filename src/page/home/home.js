...
function Home() {
  ...
  const getLaptopGaming = async () => {
    await Instance.get("/laptop-gaming")
      .then((res) => {
  ...
  const getLaptopOffice = async () => {
    await Instance.get("/laptop-office")
      .then((res) => {
  ...
  const renderListProduct = (product) => {
    ...
    // console.log(brand)
    return (
      <Card.Grid style={gridStyle} key={product.id} className="relative">
        <div
    ...
    <div className="bg-[#f0f0f0]">
      ...
      <div className="pb-[30px]">
        <div className="w-10/12 m-auto">
          ...
          >
            <h2
              className="px-[10px] py-[10px] bg-[#c8191f] text-white
          ...
          >
            ...
            <Link
              to={"/laptop/category=laptop-vanphong"}
              className="font-bold text-[#c8191f]"
            >
              {"xem tất cả >>"}
            </Link>
